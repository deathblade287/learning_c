...
void insertBetween(int data, Node *header, int position) {
  ...
  position -=
      2; // it starts from 0 so -1 and add to the next element so another -1
...
Node *append(int data, int position, Node *header) {
  if (position == 1) {
    header = insertAtStart(data, header);
  } else if (position == getLength(header) + 1) {
    insertAtEnd(data, header);
  } else {
    insertBetween(data, header, position);
  }
  return header;
}
...
int main() {
  ...
  header = append(1, 1, header);
  header = append(2, 2, header);
  header = append(3, 3, header);
  header = append(5, 4, header);
  header = append(4, 4, header);
